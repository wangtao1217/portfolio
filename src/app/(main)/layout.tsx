...
function SocialIcon({ href, icon: Icon, label }: { href: string; icon: React.ElementType; label: string }) {
  return (
    <a
      ...
      aria-label={label}
      className="w-8 h-8 rounded-full flex items-center justify-center border border-background/20 hover:border-background/50 hover:bg-background/10 transition-all duration-300"
    >
      <Icon className="h-4 w-4 text-background/60 hover:text-background" />
    </a>
  );
}
