...
function SocialIcon({ href, icon: Icon, label }: { href: string; icon: React.ElementType; label: string }) {
  return (
    <a
      ...
      aria-label={label}
      className="w-8 h-8 rounded-full flex items-center justify-center border border-[#84cc16]/30 hover:border-[#84cc16] hover:bg-[#84cc16]/10 transition-all duration-300"
    >
      <Icon className="h-4 w-4 text-[#84cc16]/60 hover:text-[#84cc16]" />
    </a>
  );
}
        ...
        <div className="flex h-14 items-center justify-between max-w-[1200px] mx-auto px-3 sm:px-5 w-full">
          ...
          <div className="flex items-center gap-3">
            {isArticlePage && (
              <Link
                href="/content"
                className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-[#84cc16]/10 transition-all duration-300"
                aria-label="返回"
              >
                <ArrowLeft className="h-4 w-4 text-[#84cc16]" />
              </Link>
            )}
            <ThemeToggle />
          </div>
